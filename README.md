...
10. Integré el SDK de Anthropic reemplazando el mock por llamadas reales a `claude-opus-4-7`, cargando la API key desde `.env` y actualizando el system prompt para Bircle Outdoors. La api key queda segura en mi archivo env, el cual no se sube a github para no exponerla públicamente.
7. Desarrollé el frontend con Streamlit usando `st.chat_input` y `st.chat_message`, con manejo de errores de conexión al backend. Streamlit no interactúa directamente con la api key, sino que realiza peticiones post al backend, y el backend es el que está conectado con la apikey del llm.
